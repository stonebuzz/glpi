   ...
   static function raiseEvent($event, $item, $options=array(), $label='') {
      ...
      if ($CFG_GLPI["use_mailing"]) {
         ...
         foreach (Notification::getNotificationsByEventAndType($event, $item->getType(), $entity)
            ...
            // Define actors types variables && actors notification booleans (lightens the code horizontaly)
            ...
            foreach ($targets as $target) {
             ...
             // If this target and all of its users are blocked, break loop
               ...
               if      ( $target['items_id'] == $group_assign    && $notify_group_assign     == 0 && $notify_user_assign     == 0 ) { break; }
               else if ( $target['items_id'] == $group_requester && $notify_group_requester  == 0 && $notify_user_requester  == 0 ) { break; }
               else if ( $target['items_id'] == $group_observer  && $notify_group_observer   == 0 && $notify_user_observer   == 0 ) { break; }
              ...
              foreach ($roles as $role) {
                ...
                foreach ($role as $actor) {
                  ...
                  // If current actor or his group isn't blocked, keep $unset at "false". Else, switch $unset to "true"

                  if      ( $actor['type'] == $actor_assign    && $notify_user_assign     == 0 && $notify_group_assign    == 0 ) { $unset = 1; }
                  else if ( $actor['type'] == $actor_assign    && $notify_supplier_assign == 0 && $notify_group_assign    == 0 ) { $unset = 1; }
                  else if ( $actor['type'] == $actor_requester && $notify_user_requester  == 0 && $notify_group_requester == 0 ) { $unset = 1; }
                  else if ( $actor['type'] == $actor_observer  && $notify_user_observer   == 0 && $notify_group_observer  == 0 ) { $unset = 1; }
