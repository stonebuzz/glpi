...
HTML;
   ...
   public static function getDCRoomPosition(CommonDBTM $item, $options = []) {

      $loc = new Location();

      $params['with_item_location']       = false;
      $params['with_rack_location']       = false;
      $params['with_dcroom_location']     = false;
      $params['with_datacenter_location'] = false;
      ...
      $itemLocation = "";
      if ($params['with_item_location'] && $item->getLocationID()) {
         $loc->getFromDB($item->getLocationID());
         $itemLocation = "(".Html::link($loc->getField('name'), Location::getFormURLWithID($item->getLocationID())).")";
      }

      $breadcrumb = Html::link($item->getField('name'), $item::getFormURLWithID($item->getID())). " ".$itemLocation." > ";

      //try to find item from Rack
      $item_rack = new Item_Rack();
      if ($item_rack->getFromDBByCrit(["itemtype" => $item->getType(), "items_id" => $item->getID()])) {
         $rack = new Rack();
         if ($rack->getFromDB($item_rack->fields['racks_id'])) {

            $dcroom = new DCRoom();
            if ($dcroom->getFromDB($rack->fields['dcrooms_id'])) {

               $datacenter = new Datacenter();
               if ($datacenter->getFromDB($dcroom->fields['datacenters_id'])) {

                  $datacenterLocation = "";
                  if ($params['with_datacenter_location'] && $datacenter->getLocationID()) {
                     $loc->getFromDB($datacenter->getLocationID());
                     $datacenterLocation = "(".Html::link($loc->getField('name'), Location::getFormURLWithID($datacenter->getLocationID())).")";
                  }

                  $breadcrumb .= Html::link($datacenter->getField('name'), Datacenter::getFormURLWithID($datacenter->getID()))." ".$datacenterLocation." > ";
               }

               $dcroomLocation = "";
               if ($params['with_dcroom_location'] && $dcroom->getLocationID()) {
                  $loc->getFromDB($dcroom->getLocationID());
                  $dcroomLocation = "(".Html::link($loc->getField('name'), Location::getFormURLWithID($dcroom->getLocationID())).")";
               }

               $breadcrumb .= Html::link($dcroom->getField('name'), DCRoom::getFormURLWithID($dcroom->getID()))." ".$dcroomLocation." > ";
            }

            $rack_location = "";
            if ($rack->isLocationAssign() && $params['with_rack_location']) {
               $loc->getFromDB($rack->getLocationID());
               $rack_location .= "(".Html::link($loc->getField('name'), Location::getFormURLWithID($rack->getLocationID())).")";
            }

            $breadcrumb .= Html::link($rack->getField('name'), Rack::getFormURLWithID($rack->getID()))." ".$rack_location." > ";
         }
         $breadcrumb .= $item_rack->fields['position'];
      }
